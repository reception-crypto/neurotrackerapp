...
function readRows() {
  ...
  return parsed.map(values => {
    ...
    row.ScoreNumber = Number(row.Score || 0);
    row.WellnessNumber = Number(row.WellnessPercent || 0);
    return row;
  });
}
...
function patientSeries(rows, disorder, metric, aggregation='weekly', selectedPatients=[]) {
  const filtered = rows.filter(r => (!disorder || r.Disorder === disorder) && (!selectedPatients.length || selectedPatients.includes(r.Patient)));
...
app.get('/admin',requireAdmin,(req,res)=>{
  ...
  const metric=req.query.metric==='symptom'?'symptom':'wellness';
