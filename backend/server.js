...
function csvFileSignature(stats) {
  return [
    stats.dev,
    stats.ino,
    stats.size,
...
function readRows() {
  ensureCsvFile();
  const before = fs.statSync(csvPath, { bigint: true });
  const signature = csvFileSignature(before);
  ...
  const after = fs.statSync(csvPath, { bigint: true });
  const finalSignature = csvFileSignature(after);
...
app.post(
  ...
  const profileDisorders=isIndependentClinicalProfile(assignedProfile)
    ...
    : '';
  const newRows=acceptedRecords.map(r=>normalisedRecord(
      receivedAt,
      b.date,
      b.time,
      patientName,
  ...
  const updatedCsv = fs.statSync(csvPath, { bigint: true });
  rowCache = {
    signature: csvFileSignature(updatedCsv),
    rows: [
      ...existingRows,
      ...newRows.map(row => ({
        ...row,
