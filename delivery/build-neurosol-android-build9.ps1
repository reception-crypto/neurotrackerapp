...
if ($LASTEXITCODE -ne 0 -or $sourceCommit -notmatch '^[0-9a-f]{40}$') {
    throw 'Could not determine the Build 9 source commit.'
}
$requiredAncestors = @(
    '1c9a365', # independent symptom/disorder architecture
    'f2adba0', # Build 8 visual identity retained by Build 9
...
if ($LASTEXITCODE -ne 0 -or $sourceStatus.Count -ne 0) {
    throw (
        'Build 9 source or release tooling is not committed. Commit the ' +
...
if ((Get-Content -LiteralPath $pubspecPath -Raw) -notmatch
    '(?m)^version:\s*1\.2\.0\+9\s*$') {
    throw 'pubspec.yaml is not set to version 1.2.0+9.'
}
...
foreach ($requiredIdentity in @(
    ...
    if ($identityText -notmatch [regex]::Escape($requiredIdentity)) {
        throw "The Build 9 mobile identity is missing: $requiredIdentity"
    }
}
$package = Get-Content `
    -LiteralPath (Join-Path $projectRoot 'backend\package.json') `
    -Raw | ConvertFrom-Json
if ([string]$package.version -ne '0.11.0') {
    throw "Expected backend 0.11.0, found $($package.version)."
}
...
foreach ($requiredAsset in @(
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $projectRoot $requiredAsset))) {
        throw "A Build 9 visual-identity asset is missing: $requiredAsset"
    }
}
$mobileSource = @(
...
foreach ($requiredMarker in @(
    ...
    if ($mobileSource -notmatch [regex]::Escape($requiredMarker)) {
        throw "A reconciled Build 9 mobile feature is missing: $requiredMarker"
    }
}
...
foreach ($requiredMarker in @(
    ...
    if ($backendSource -notmatch [regex]::Escape($requiredMarker)) {
        throw "A reconciled Build 9 backend feature is missing: $requiredMarker"
    }
}
...
Write-Host 'Analyzing Build 9...'
Invoke-Checked `
    -Command 'flutter' `
    -Arguments @('analyze') `
    -Description 'Flutter analysis'
Write-Host 'Running backend 0.11.0 tests...'
...
Invoke-Checked `
    ...
    -Arguments @(
        ...
        (Join-Path `
            $projectRoot `
            'delivery\build9-backend-compatibility\test\verify-clinical-data.test.js')
    ) `
...
if ($LASTEXITCODE -ne 0 -or $postVerificationStatus.Count -ne 0) {
    throw (
        'Formatting or verification changed the committed Build 9 source. ' +
        'Review and commit those changes before building signed artifacts.'
    )
}

Write-Host 'Building signed Build 9 Android App Bundle...'
Invoke-Checked `
    -Command 'flutter' `
    -Arguments @(
        'build',
    ...
    -Description 'Signed Build 9 App Bundle'
Write-Host 'Building signed Build 9 Android APK...'
Invoke-Checked `
    -Command 'flutter' `
    -Arguments @(
        'build',
    ...
    -Description 'Signed Build 9 APK'
...
$releaseDirectory = Join-Path $projectRoot 'delivery\android-build9'
$releaseAab = Join-Path `
    $releaseDirectory `
    'NeuroSol-Symptom-Diary-1.2.0-build9.aab'
$releaseApk = Join-Path `
    $releaseDirectory `
    'NeuroSol-Symptom-Diary-1.2.0-build9.apk'
...
foreach ($target in @($releaseAab, $releaseApk, $metadataPath)) {
    if ((Test-Path -LiteralPath $target) -and -not $Force) {
        throw "A Build 9 release artifact already exists: $target"
    }
}
...
Write-Host ''
Write-Host 'NEUROSOL ANDROID BUILD 9 COMPLETE' -ForegroundColor Green
...
Write-Host '$Apk = (Resolve-Path ''.\delivery\android-build9\NeuroSol-Symptom-Diary-1.2.0-build9.apk'').Path'
