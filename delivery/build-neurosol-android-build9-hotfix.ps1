...
if ($LASTEXITCODE -ne 0 -or $sourceCommit -notmatch '^[0-9a-f]{40}$') {
    throw 'Could not determine the Build 9 hotfix source commit.'
}
$requiredAncestors = @(
    ...
    'c9e8b28'  # iOS Large Text wellness-submit fix and regression test
)
...
$sourceStatus = @(
    & git -C $projectRoot status --porcelain -- `
        ...
        README.md `
        STORE_RELEASE_PREP.md `
        HOTFIX_RELEASE_CHECKLIST.md `
        delivery/CODEMAGIC_BUILD9_HOTFIX.md `
        delivery/build-neurosol-android-build9-hotfix.ps1 `
...
if ($LASTEXITCODE -ne 0 -or $sourceStatus.Count -ne 0) {
    throw (
        'Build 9 hotfix source or release tooling is not committed. Commit the ' +
...
if ((Get-Content -LiteralPath $pubspecPath -Raw) -notmatch
    '(?m)^version:\s*1\.1\.1\+9\s*$') {
    throw 'pubspec.yaml is not set to version 1.1.1+9.'
}
...
foreach ($requiredIdentity in @(
    ...
    if ($identityText -notmatch [regex]::Escape($requiredIdentity)) {
        throw "The Build 9 hotfix mobile identity is missing: $requiredIdentity"
    }
}
$package = Get-Content `
...
foreach ($requiredAsset in @(
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $projectRoot $requiredAsset))) {
        throw "A Build 9 hotfix visual-identity asset is missing: $requiredAsset"
    }
}
$mobileSource = @(
...
foreach ($requiredMarker in @(
    ...
    'schemaVersion',
    'profileDisorderIds',
    'wellness-content-scroll',
    'submit-wellness-check-in'
)) {
    if ($mobileSource -notmatch [regex]::Escape($requiredMarker)) {
        throw "A required Build 9 hotfix mobile feature is missing: $requiredMarker"
    }
}
...
Write-Host 'Analyzing Build 9 hotfix...'
Invoke-Checked `
    -Command 'flutter' `
    -Arguments @('analyze') `
...
if ($LASTEXITCODE -ne 0 -or $postVerificationStatus.Count -ne 0) {
    throw (
        'Formatting or verification changed the committed Build 9 hotfix source. ' +
        'Review and commit those changes before building signed artifacts.'
    )
}

Write-Host 'Building signed Build 9 hotfix Android App Bundle...'
Invoke-Checked `
    -Command 'flutter' `
    -Arguments @(
        'build',
    ...
    -Description 'Signed Build 9 hotfix App Bundle'
Write-Host 'Building signed Build 9 hotfix Android APK...'
Invoke-Checked `
    -Command 'flutter' `
    -Arguments @(
        'build',
    ...
    -Description 'Signed Build 9 hotfix APK'
...
$releaseDirectory = Join-Path $projectRoot 'delivery\android-build9-hotfix'
$releaseAab = Join-Path `
    $releaseDirectory `
    'NeuroSol-Symptom-Diary-1.1.1-build9.aab'
$releaseApk = Join-Path `
    $releaseDirectory `
    'NeuroSol-Symptom-Diary-1.1.1-build9.apk'
...
foreach ($target in @($releaseAab, $releaseApk, $metadataPath)) {
    if ((Test-Path -LiteralPath $target) -and -not $Force) {
        throw "A Build 9 hotfix release artifact already exists: $target"
    }
}
...
[ordered]@{
    ...
    versionName = '1.1.1'
    buildNumber = 9
    releasePurpose = 'ios-large-text-submit-accessibility-hotfix'
...
Write-Host ''
Write-Host 'NEUROSOL ANDROID BUILD 9 HOTFIX COMPLETE' -ForegroundColor Green
...
Write-Host '$Apk = (Resolve-Path ''.\delivery\android-build9-hotfix\NeuroSol-Symptom-Diary-1.1.1-build9.apk'').Path'
