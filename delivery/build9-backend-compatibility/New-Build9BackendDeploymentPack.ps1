...
$requiredAncestors = @(
    ...
    'f2adba0', # Build 9 visual identity
...
if ($packageJson.version -ne '0.11.0') {
    throw "Expected backend version 0.11.0, found $($packageJson.version)."
}
...
$packageName = "NeuroSol-Build9-Compatible-Backend-$shortCommit"
...
try {
    ...
    Copy-Item `
        -LiteralPath (Join-Path $repositoryRoot 'BUILD9_RELEASE_CHECKLIST.md') `
    ...
    $release = [ordered]@{
        ...
        appStoreUrl = 'https://apps.apple.com/app/id6796575355'
        minimumMobileBuild = 7
        latestMobileBuildDuringBackendPredeployment = 8
        customDisordersEnabledDuringBackendPredeployment = $true
        independentProfilesEnabledDuringBackendPredeployment = $true
