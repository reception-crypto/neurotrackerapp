...
$script:NeuroSolExpectedBackendVersion = '0.11.0'
...
function Set-NeuroSolDotEnvValues {
    ...
    $temporaryPath = "$Path.build9-$transactionId.tmp"
    $replacementBackupPath = "$Path.build9-$transactionId.bak"
    try {
...
function Get-NeuroSolMobileConfig {
    ...
    if ($Diary) {
        $headers['X-NeuroSol-Diary'] = 'patient-diary-v1'
    }
...
function Assert-NeuroSolCompatibilityResponses {
    ...
    if (
        ...
        $health.patientDiaryAvailable -ne $true -or
        [int]$health.maximumBackdateDays -ne 7 -or
        $health.enrolmentIncidentLockdown -ne $false
    ) {
        throw "Unexpected Build 9 health configuration at $BaseUri."
    }
    ...
    if (
        ...
        $build7.independentProfilesEnabled -ne $true -or
        $build7.patientDiary -ne $true -or
        [int]$build7.maximumBackdateDays -ne 7 -or
    ...
    if (
        ...
        [int]$build8.latestBuild -ne 8 -or
        [int]$build8.disorderCatalogVersion -ne 3 -or
        [int]$build8.preferredPayloadSchemaVersion -ne 3 -or
        $build8.canonicalDisorders -ne $true -or
        $build8.independentProfileModel -ne $true -or
        $build8.independentProfilesEnabled -ne $true -or
        [int]$build8.maximumProfileSymptoms -ne 6 -or
        $build8.customDisordersEnabled -ne $true
    ) {
        throw "Build 8 compatibility verification failed at $BaseUri."
    }

    $build9 = Get-NeuroSolMobileConfig `
        -BaseUri $BaseUri `
        -Build 9 `
        -Independent `
        -Diary
    if (
        [int]$build9.minimumBuild -ne 7 -or
        [int]$build9.latestBuild -ne 8 -or
        [int]$build9.preferredPayloadSchemaVersion -ne 3 -or
        $build9.independentProfileModel -ne $true -or
        $build9.independentProfilesEnabled -ne $true -or
        $build9.patientDiary -ne $true -or
        [int]$build9.maximumBackdateDays -ne 7 -or
        [int]$build9.maximumProfileSymptoms -ne 6
    ) {
        throw "Build 9 backend predeployment gate failed at $BaseUri."
    }
}

function Assert-NeuroSolBuild9ActiveResponses {
    param(
        [Parameter(Mandatory = $true)][string]$BaseUri,
        [int]$TimeoutSeconds = 30
    )
    ...
    if (
        ...
        throw "Unexpected active Build 9 health configuration at $BaseUri."
    }
    ...
    if (
        ...
        $build7.independentProfilesEnabled -ne $true -or
        $build7.patientDiary -ne $true -or
        [int]$build7.maximumBackdateDays -ne 7
    ) {
        throw "Build 7 compatibility failed after Build 9 activation at $BaseUri."
    }
    ...
    if (
        ...
        [int]$build8.preferredPayloadSchemaVersion -ne 3 -or
        $build8.canonicalDisorders -ne $true -or
        $build8.customDisordersEnabled -ne $true -or
        $build8.independentProfilesEnabled -ne $true
    ) {
        throw "Build 8 compatibility failed after Build 9 activation at $BaseUri."
    }
    ...
    if (
        [int]$build9.minimumBuild -ne 7 -or
        [int]$build9.latestBuild -ne 9 -or
        [int]$build9.preferredPayloadSchemaVersion -ne 3 -or
        $build9.independentProfileModel -ne $true -or
        $build9.customDisordersEnabled -ne $true -or
        $build9.independentProfilesEnabled -ne $true -or
        $build9.patientDiary -ne $true -or
        [int]$build9.maximumBackdateDays -ne 7 -or
        [int]$build9.maximumProfileSymptoms -ne 6
    ) {
        throw "Build 9 activation failed at $BaseUri."
    }
}
