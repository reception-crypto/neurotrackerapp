...
function snapshotData(dataDirectory) {
  ...
  return {
    ...
    bpPatientIdCount: patients.filter(
      patient => String(patient?.bpPatientId || '').trim(),
    ).length,
...
function compareAfterMigration(before, after) {
  ...
  assertEqual(before, after, 'bpPatientIdCount', 'BP Patient ID count');
  ...
  if (after.identityStoreVersion < 4) {
    throw new Error('The migrated identity-store version is below 4.');
  }
  ...
  if (!after.disorderCatalogPresent || after.disorderCatalogVersion !== 3) {
    throw new Error('The Build 9 disorder catalogue was not initialised safely.');
  }
