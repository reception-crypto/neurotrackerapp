    ...
    try {
        ...
        try {
            Invoke-NodeCommand `
                ...
                -Description 'Isolated Build 7/8/9 backend test suite'
        } finally {
            Pop-Location
        }
    } finally {
...
if ($release.backendVersion -ne '0.11.0') {
    throw "Expected backend version 0.11.0, found $($release.backendVersion)."
}
if (
...
if (
    ...
    [int]($release.latestMobileBuildDuringBackendPredeployment) -ne 8 -or
    $release.customDisordersEnabledDuringBackendPredeployment -ne $true -or
    $release.independentProfilesEnabledDuringBackendPredeployment -ne $true -or
    [int]($release.maximumBackdateDays) -ne 7
) {
    throw 'The release metadata does not preserve the Build 7/8 rollout.'
}
if (
...
try {
    ...
    Write-Host 'Deploying the Build 9 compatibility backend...'
    foreach ($relativePath in $managedFiles) {
    ...
    Write-Host 'Verifying local Build 7, Build 8 and gated Build 9 responses...'
    ...
    if (-not $SkipPublicVerification) {
        Write-Host 'Verifying the public HTTPS Build 7/8/9 path...'
    ...
    Write-Host ''
    Write-Host 'Build 9 compatibility backend deployed successfully.'
    Write-Host "Source commit: $($release.sourceCommit)"
    Write-Host 'Build 7 support: ACTIVE'
    Write-Host 'Build 8 support: ACTIVE'
    Write-Host 'Build 9 diary/backdating backend: READY but not advertised'
    ...
    throw "Build 9 compatibility deployment failed: $($deploymentError.Exception.Message)$restoreMessage"
}
