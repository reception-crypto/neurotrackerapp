...
if ($Confirmation -cne 'ACTIVATE BUILD 8 AFTER BOTH STORES ARE LIVE') {
    throw "Supply -Confirmation 'ACTIVATE BUILD 8 AFTER BOTH STORES ARE LIVE' exactly."
}
...
foreach ($relativePath in $sourceFiles) {
    ...
    if (
        ...
        throw "Build 8 source verification is missing: $relativePath"
    }
...
if (
    ...
    throw 'Build 8 cannot be activated while enrolment incident lockdown is active or undefined.'
}
...
if ($alreadyActive) {
    Assert-NeuroSolBuild8ActiveResponses `
    ...
    Write-Host 'Build 8 is already active and passed verification.'
    return
}
...
if ($service.Status -ne 'Running') {
    throw "Service '$ServiceName' must be running before Build 8 activation."
}
...
try {
    ...
    if (
        ...
        throw 'A protected production secret changed during Build 8 activation.'
    }
    ...
    Write-Host ''
    Write-Host 'NEUROSOL BUILD 8 ACTIVATED' -ForegroundColor Green
    ...
    try {
        if ($backupReady) {
            ...
            Assert-NeuroSolCompatibilityResponses `
                ...
                -TimeoutSeconds $StartupTimeoutSeconds
            Write-Host (
                'Build 8 activation was rolled back; Build 7 compatibility is active.'
