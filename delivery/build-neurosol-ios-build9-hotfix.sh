...
if [[ $(uname -s) != Darwin ]]; then
  echo 'The signed iOS Build 9 hotfix script must run on a CodeMagic macOS runner.' >&2
  exit 1
fi
...
if [[ ! $source_commit =~ ^[0-9a-f]{40}$ ]]; then
  echo 'Could not determine the Build 9 hotfix source commit.' >&2
  exit 1
fi
required_ancestors=(
  1c9a365
...
release_directory="$project_root/delivery/ios-build9-hotfix"
release_ipa="$release_directory/NeuroSol-Symptom-Diary-1.1.1-build9.ipa"
metadata_path="$release_directory/release.json"
if [[ $force == false ]]; then
  for target in "$release_ipa" "$metadata_path"; do
    if [[ -e $target ]]; then
      echo "A Build 9 hotfix release artifact already exists: $target" >&2
...
if [[ -n $source_status ]]; then
  echo 'Build 9 hotfix source or release tooling is not committed:' >&2
  printf '%s\n' "$source_status" >&2
  exit 1
fi

grep -Eq '^version:[[:space:]]*1\.1\.1\+9[[:space:]]*$' pubspec.yaml
grep -Fq "appVersion = '1.1.1'" lib/app_identity.dart
...
grep -Fq 'wellness-content-scroll' lib/screens/wellness_screen.dart
grep -Fq 'submit-wellness-check-in' lib/screens/wellness_screen.dart
...
if [[ -n $post_verification_status ]]; then
  echo \
    'Formatting or verification changed the committed Build 9 hotfix source. Review and commit those changes before building.' \
    >&2
  printf '%s\n' "$post_verification_status" >&2
  exit 1
fi

echo 'Building signed Build 9 hotfix iOS archive and IPA...'
flutter build ipa \
  --release \
  --build-name=1.1.1 \
  --build-number=9 \
...
temporary_root=$(mktemp -d "${TMPDIR:-/tmp}/neurosol-build9-hotfix.XXXXXX")
...
if [[ $built_bundle_id != au.com.pascoeneurology.neurosol || \
      $built_version != 1.1.1 || $built_number != 9 ]]; then
  echo \
...
echo
echo 'NEUROSOL IOS BUILD 9 HOTFIX COMPLETE'
