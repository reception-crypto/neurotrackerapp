...
if ((Get-Content $pubspecPath -Raw) -notmatch '(?m)^version:\s*1\.0\.0\+4\s*$') {
    throw 'pubspec.yaml is not set to version 1.0.0+4.'
}
...
$releaseDirectory = Join-Path $projectRoot 'delivery\android-build4'
New-Item -ItemType Directory -Path $releaseDirectory -Force | Out-Null

$releaseAab = Join-Path $releaseDirectory 'NeuroSol-Symptom-Diary-1.0.0-build4.aab'
$releaseApk = Join-Path $releaseDirectory 'NeuroSol-Symptom-Diary-1.0.0-build4.apk'

Copy-Item $sourceAab $releaseAab -Force
Copy-Item $sourceApk $releaseApk -Force

Write-Host ''
Write-Host 'Build 4 completed successfully.'
...
Write-Host '$Apk = (Resolve-Path ".\delivery\android-build4\NeuroSol-Symptom-Diary-1.0.0-build4.apk").Path'
Write-Host 'flutter install --release --use-application-binary="$Apk"'
